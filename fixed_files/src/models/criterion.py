...
class RTDETRCriterion(nn.Module):
    ...
    def forward(
        ...
        if True:  # Always log
            ...
            print(f"[DEBUG] Loss components: {loss_str}", file=sys.stderr, flush=True)
            total = sum(losses.values())
            print(f"[DEBUG] Total loss: {total.item():.4f}, num_boxes: {num_boxes}", file=sys.stderr, flush=True)
        return losses
    ...
    def loss_labels(
        ...
        if len(target_classes_o) > 0:
            ...
            if len(target_classes_o) > 0:
                min_label = target_classes_o.min().item()
                max_label = target_classes_o.max().item()
                if min_label < 0 or max_label >= target_scores.size(2):
                    raise ValueError(f"Label out of bounds: labels=[{min_label}, {max_label}], target_scores.shape={target_scores.shape}")
            target_scores[idx[0], idx[1], target_classes_o.long()] = iou.detach().to(target_scores.dtype)
        ...
        loss_vfl = self.vfl(
            ...
            target_classes_clamped.flatten(0, 1),
